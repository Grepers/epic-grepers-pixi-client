...
class Loader {
	...
	constructor() {
		...
		this._assetLoader.add( '/img/soldier.png' );
	...
	load( deepstreamUrl, callback ) {
		...
		global.ws = new WebSocket('ws://' + deepstreamUrl + '/client');
		global.ws.OPEN;
		...
		global.ws.onmessage = function (event) {
			...
			global.pendingMessages.push(JSON.parse(event.data));
		}
	...
	_onImagesLoaded() {
		...
		console.log("_onImagesLoaded: _imagesReady?" + this._imagesReady);
		this._checkReady();
	}
	...
	_onLoggedIn() {
		...
		console.log("_onLoggedIn: conecctionReady?" + this._connectionReady);
		this._checkReady();
	}
	 ...
	 */
	_checkReady() {
		console.log("_checkReady: connectionReady?" + this._connectionReady);
